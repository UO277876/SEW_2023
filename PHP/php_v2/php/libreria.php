<?php
    class Libreria {
        ...
        public function export(){
            // 1. Se establece conexión con la BD + establecer las tablas de la BD
            $this->connectionBD();
            $this->db->select_db("tienda");

            $tablas = array('autor','libro','libreria','usuario','contiene','compra');

            // 2. Encabezados + apertura del archivo que se va a exportar
            header('Content-Type: text/csv');
            header('Content-Disposition: attachment; filename=total.csv');

            $fp = fopen('php://output', 'w');

            // 3. Se va poniendo los datos en el archivo de cada tabla
            foreach($tablas as $tabla){
                $consultaPre = $this->db->prepare("SELECT * FROM " . $tabla);
                $consultaPre->execute();
            ...
            // 4. Cierre archivo + cierre de la BD
            // Sin el exit exporta también el código HTML
            fclose($fp);
            $this->cerrarBD(); 
            exit;
        }
            ...
            <section>
                ...
                <form action='#' method='post' enctype='multipart/form-data'>
                    <p><label for='importUsuarios'>Cargue el CSV de usuarios:</label> 
                        <input id='importUsuarios' type='file' accept=".csv" name='usuarios'/></p>
            ...
            <section>
                ...
                <form action='#' method='post' enctype='multipart/form-data'>
                    <p><label for='importAutores'>Cargue el CSV de autores:</label> 
                        <input id='importAutores' type='file' accept=".csv" name='autores'/></p>
            ...
            <section>
                ...
                <form action='#' method='post' enctype='multipart/form-data'>
                    <p><label for='importLibrerias'>Cargue el CSV de librerías:</label> 
                        <input id='importLibrerias' type='file' accept=".csv" name='librerias'/></p>
            ...
            <section>
                ...
                <form action='#' method='post' enctype='multipart/form-data'>
                    <p><label for='importLibros'>Cargue el CSV de libros:</label> 
                        <input id='importLibros' type='file' accept=".csv" name='libros'/></p>
            ...
            <section>
                ...
                <form action='#' method='post' enctype='multipart/form-data'>
                    <p><label for='importContiene'>Cargue el CSV que contiene los libros:</label> 
                        <input id='importContiene' type='file' accept=".csv" name='contiene'/></p>
            ...
            <section>
                ...
                <form action='#' method='post' enctype='multipart/form-data'>
                    <p><label for='importCompra'>Cargue el CSV que contiene las compras:</label> 
                        <input id='importCompra' type='file' accept=".csv" name='compra'/></p>
